...
/*************************************************************************/
private fun IDBColumn<*>.generateFunction(select: Select, function: IDBParameterizedFunction): FromFunction {
    var fn: FromFunction
    ...
    if (existingFromFunction == null)
        fn = FromFunction(function, ColumnName, this.DataType.DefaultValue!!)
    else {
        select.selectors.remove(existingFromFunction)
        fn = FromFunction(function.apply { this.Value = FuncFromFunction({ existingFromFunction.Function }) }, ColumnName, this.DataType.DefaultValue!!)
    }
    return fn
...
/*Aggregate Functions*/
infix fun <T : Number, ENTITY : IDBEntity, COLLECTION> COLLECTION.sum(column: IDBColumn<T>) where COLLECTION : IQueriableCollection<ENTITY>, COLLECTION : ISelectQueryExpression = this.applyUpdate { removeFromColumnSelectorIfExists(column);this.Select.selectors.add(column.generateFunction(Select, Sum(FuncFromColumn({ column.fullColumnName })))) }
infix fun <T, ENTITY : IDBEntity, COLLECTION> COLLECTION.avg(column: IDBColumn<T>) where COLLECTION : IQueriableCollection<ENTITY>, COLLECTION : ISelectQueryExpression = this.applyUpdate { removeFromColumnSelectorIfExists(column);this.Select.selectors.add(column.generateFunction(Select, Avg(FuncFromColumn({ column.fullColumnName })))) }
infix fun <T, ENTITY : IDBEntity, COLLECTION> COLLECTION.count(column: IDBColumn<T>) where COLLECTION : IQueriableCollection<ENTITY>, COLLECTION : ISelectQueryExpression = this.applyUpdate { removeFromColumnSelectorIfExists(column);this.Select.selectors.add(column.generateFunction(Select, Count(FuncFromColumn({ column.fullColumnName })))) }
infix fun <T, ENTITY : IDBEntity, COLLECTION> COLLECTION.max(column: IDBColumn<T>) where COLLECTION : IQueriableCollection<ENTITY>, COLLECTION : ISelectQueryExpression = this.applyUpdate { removeFromColumnSelectorIfExists(column);this.Select.selectors.add(column.generateFunction(Select, Max(FuncFromColumn({ column.fullColumnName })))) }
infix fun <T, ENTITY : IDBEntity, COLLECTION> COLLECTION.min(column: IDBColumn<T>) where COLLECTION : IQueriableCollection<ENTITY>, COLLECTION : ISelectQueryExpression = this.applyUpdate { removeFromColumnSelectorIfExists(column);this.Select.selectors.add(column.generateFunction(Select, Min(FuncFromColumn({ column.fullColumnName })))) }
/*Scalar Functions*/
