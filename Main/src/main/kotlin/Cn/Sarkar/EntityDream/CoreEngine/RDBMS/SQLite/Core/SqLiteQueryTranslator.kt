...
package Cn.Sarkar.EntityDream.CoreEngine.RDBMS.SQLite.Core
...
import Cn.Sarkar.EntityDream.CoreEngine.RDBMS.SQLite.Core.Subject.NamingRuleSubject
...
class SqLiteQueryTranslator(override val DataContext: IDataContext) : IQueryTranslator {
    ...
    override fun Translate(expression: IQueryExpression): TranslateResult {
        ...
        when (expression) {
            is ISelectQueryExpression -> {
                ...
                buffer.append(expression.Select.offset.run { if (this == null) "" else " OFFSET $this" })
                buffer.append(";")
//                println(buffer)
            }
            is UpdateQueryExpression -> {

                buffer.append("UPDATE ${expression.update.TableName} SET ${expression.set.Values.joinToString { "${it.key.ColumnName}=${it.value.run { this.renderToString() }}" }} ")
